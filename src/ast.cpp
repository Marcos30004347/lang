...
AST_Id ast_manager_reserve(AST_Manager *m, Token tok, u64 kind, AST_Id l,
                           AST_Id r) {
  AST_Id id = m->size;
...
AST_Node *ast_manager_alloc(AST_Manager *m, Token tok, u64 kind, AST_Id l,
                            AST_Id r) {
...
AST_Node *ast_symbol(AST_Manager *m, Token tok) {
  return ast_manager_alloc(m, tok, AST_SYMBOL_LITERAL, 0, 0);
}

AST_Node *ast_i32_lit(AST_Manager *m, Token tok) {
  return ast_manager_alloc(m, tok, AST_NATURAL_LITERAL, 0, 0);
}
...
AST_Node *ast_call(AST_Manager *m, Token tok, AST_Node *l, AST_Node *r,
                   bool effectfull) {
  return ast_manager_alloc(m, tok, effectfull ? AST_EFFECT_CALL : AST_FUNCTION_CALL,
                           l->id, r->id);
}
...
AST_Node *ast_with_handler(AST_Manager *m, Token tok, AST_Node *call,
                       AST_Node *hnd) {
