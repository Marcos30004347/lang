...
template <> Effect_Literal_Node* is_instance<>(Node* node) {
  return node && node->kind == AST_EFFECT_LITERAL ? as< Effect_Literal_Node* >(node) : 0;
}
...
Effect_Literal_Node* create_node_effect_literal(ast::Manager* manager, Node* arguments, Node* return_type, ProgramPoint_List_Node* body) {
  ...
  return as< Effect_Literal_Node* >(ast::manager_alloc(manager, AST_EFFECT_LITERAL, get_id(signature), get_id(body)));
}
...
Node* Effect_Literal_Node::get_return_type(ast::Manager* manager) {
  Node* signature = left_of(manager, this);
  return right_of(manager, signature);
}

Declarations_List_Node* Effect_Literal_Node::get_arguments(ast::Manager* manager) {
  Node* signature = left_of(manager, this);
...
void Effect_Literal_Node::push_argument(ast::Manager* manager, ast::Declaration_Variable_Node* arg) {
  Node* signature = left_of(manager, this);
