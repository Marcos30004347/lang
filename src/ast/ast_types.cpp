...
template <> Type_Pointer_Node* is_instance<>(Node* node) {
  return node && node->kind == AST_TYPE_ARROW ? as< Type_Pointer_Node* >(node) : 0;
}
...
Type_Pointer_Node* create_node_type_pointer(ast::Manager* manager, Node* type) {
  assert(type);
  return as< Type_Pointer_Node* >(ast::manager_alloc(manager, AST_TYPE_POINTER, type->id, 0));
}
...
Node* Type_Pointer_Node::get_type(ast::Manager* manager) {
