...
Arithmetic_Operation_Add_Node* create_node_arithmetic_add(parser::Parser* parser, Node* a, Node* b) {
  assert(a && b);
  return as< Arithmetic_Operation_Add_Node* >(
      ast::manager_alloc(parser->ast_manager, AST_OP_BIN_ADD, a->id, b->id));
}

Arithmetic_Operation_Sub_Node* create_node_arithmetic_sub(parser::Parser* parser, Node* a, Node* b) {
  assert(a && b);
  return as< Arithmetic_Operation_Sub_Node* >(
      ast::manager_alloc(parser->ast_manager, AST_OP_BIN_SUB, a->id, b->id));
}

Arithmetic_Operation_Mul_Node* create_node_arithmetic_mul(parser::Parser* parser, Node* a, Node* b) {
  assert(a && b);
  return as< Arithmetic_Operation_Mul_Node* >(
      ast::manager_alloc(parser->ast_manager, AST_OP_BIN_MUL, a->id, b->id));
}

Arithmetic_Operation_Div_Node* create_node_arithmetic_div(parser::Parser* parser, Node* a, Node* b) {
  assert(a && b);
  return as< Arithmetic_Operation_Div_Node* >(
      ast::manager_alloc(parser->ast_manager, AST_OP_BIN_DIV, a->id, b->id));
}

Logical_Operation_Greater_Node* create_node_logical_greater_than(parser::Parser* parser, Node* a, Node* b) {
  assert(a && b);
  return as< Logical_Operation_Greater_Node* >(
      ast::manager_alloc(parser->ast_manager, AST_OP_BIN_GT, a->id, b->id));
}

Logical_Operation_GreaterEqual_Node*
create_node_logical_greater_equal_than(parser::Parser* parser, Node* a, Node* b) {
  assert(a && b);
  return as< Logical_Operation_GreaterEqual_Node* >(
      ast::manager_alloc(parser->ast_manager, AST_OP_BIN_GE, a->id, b->id));
}
...
Logical_Operation_LessEqual_Node* create_node_logical_less_equal_than(parser::Parser* parser, Node* a, Node* b) {
  assert(a && b);
  return as< Logical_Operation_LessEqual_Node* >(
      ast::manager_alloc(parser->ast_manager, AST_OP_BIN_LE, a->id, b->id));
}
...
Logical_Operation_NotEqual_Node* create_node_logical_not_equal_to(parser::Parser* parser, Node* a, Node* b) {
  assert(a && b);
  return as< Logical_Operation_NotEqual_Node* >(
      ast::manager_alloc(parser->ast_manager, AST_OP_BIN_NE, a->id, b->id));
}
...
Node* Variable_Assignment_Node::get_left_operand(parser::Parser* parser) {
  return left_of(parser->ast_manager, this);
}

Node* Variable_Assignment_Node::get_right_operand(parser::Parser* parser) {
  return right_of(parser->ast_manager, this);
}
