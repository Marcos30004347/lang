...
u8 lexer_skip_whitespaces(Lexer *t) {
  while (!lexer_is_eof(t) &&
         (lexer_current(t) == ' ' || lexer_current(t) == '\t')) {
    lexer_eat(t);
  }
...
Token token_create(Token_Type ty, u64 pos, u64 col, u64 row, u64 file, u64 size,
                   u64 buf = 0) {
  Token tok;
...
Token lexer_read_i32lit(Lexer *t) {
  ...
  Token tok =
      token_create(TOKEN_I32_LIT, pos, col, row, t->file_id, t->file_pos - s);
...
b8 lexer_can_read_word(Lexer *t, const char *word, u64 *size = 0) {
  ...
  while (word[i] != '\0' && !lexer_is_eof(t) &&
         t->file_buf[t->file_pos + i] == word[i]) {
    i = i + 1;
  }
...
Token eof(Lexer *lex) {
  return token_create(TOKEN_EOF, lex->file_pos, lex->file_col, lex->file_row,
                      lex->file_id, 0);
}
...
Token lexer_read_token(Lexer *t) {
  ...
  if (lexer_current(t) == '\'' && lexer_eat(t))
    return lexer_set_curr(
        t, token_create(TOKEN_APHOSTROPHE, pos, col, row, t->file_id, 1));
  if (lexer_current(t) == ':' && lexer_eat(t))
    return lexer_set_curr(
        t, token_create(TOKEN_COLON, pos, col, row, t->file_id, 1));
  if (lexer_current(t) == ',' && lexer_eat(t))
    return lexer_set_curr(
        t, token_create(TOKEN_COMMA, pos, col, row, t->file_id, 1));
  if (lexer_current(t) == '!' && lexer_eat(t))
    return lexer_set_curr(
        t, token_create(TOKEN_EXCLAMATION, pos, col, row, t->file_id, 1));
  if (lexer_current(t) == ';' && lexer_eat(t))
    return lexer_set_curr(
        t, token_create(TOKEN_SEMI_COLON, pos, col, row, t->file_id, 1));
  if (lexer_current(t) == '.' && lexer_eat(t))
    return lexer_set_curr(
        t, token_create(TOKEN_DOT, pos, col, row, t->file_id, 1));
  if (lexer_current(t) == '{' && lexer_eat(t))
    return lexer_set_curr(
        t, token_create(TOKEN_OPEN_CURLY_BRACE, pos, col, row, t->file_id, 1));
  if (lexer_current(t) == '}' && lexer_eat(t))
    return lexer_set_curr(
        t, token_create(TOKEN_CLOSE_CURLY_BRACE, pos, col, row, t->file_id, 1));
  if (lexer_current(t) == '(' && lexer_eat(t))
    return lexer_set_curr(
        t, token_create(TOKEN_OPEN_PARENTHESIS, pos, col, row, t->file_id, 1));
  if (lexer_current(t) == ')' && lexer_eat(t))
    return lexer_set_curr(
        t, token_create(TOKEN_CLOSE_PARENTHESIS, pos, col, row, t->file_id, 1));

  if (lexer_current(t) == '+' && lexer_eat(t))
    return lexer_set_curr(
        t, token_create(TOKEN_PLUS, pos, col, row, t->file_id, 1));

  if (lexer_current(t) == '-' && lexer_eat(t)) {
    if (lexer_current(t) == '>' && lexer_eat(t)) {
      return lexer_set_curr(
          t, token_create(TOKEN_ARROW, pos, col, row, t->file_id, 2));
    }

    return lexer_set_curr(
        t, token_create(TOKEN_MINUS, pos, col, row, t->file_id, 1));
  }

  if (lexer_current(t) == '|' && lexer_eat(t))
    return lexer_set_curr(
        t, token_create(TOKEN_PIPE, pos, col, row, t->file_id, 1));

  if (lexer_current(t) == '*' && lexer_eat(t))
    return lexer_set_curr(
        t, token_create(TOKEN_ASTERISK, pos, col, row, t->file_id, 1));

  if (lexer_current(t) == '/' && lexer_eat(t))
    return lexer_set_curr(
        t, token_create(TOKEN_SLASH, pos, col, row, t->file_id, 1));

  if (lexer_current(t) == '<' && lexer_eat(t)) {
    if (lexer_current(t) == '=' && lexer_eat(t)) {
      return lexer_set_curr(
          t, token_create(TOKEN_SMALLER_EQUAL, pos, col, row, t->file_id, 2));
    }

    return lexer_set_curr(
        t, token_create(TOKEN_SMALLER, pos, col, row, t->file_id, 1));
  }

  if (lexer_current(t) == '>' && lexer_eat(t)) {
    if (lexer_current(t) == '=' && lexer_eat(t)) {
      return lexer_set_curr(
          t, token_create(TOKEN_GREATER_EQUAL, pos, col, row, t->file_id, 2));
    }

    return lexer_set_curr(
        t, token_create(TOKEN_GREATER, pos, col, row, t->file_id, 1));
  }

  if (lexer_current(t) == '=' && lexer_eat(t)) {
    if (lexer_current(t) == '=' && lexer_eat(t)) {
      return lexer_set_curr(
          t, token_create(TOKEN_EQUAL_EQUAL, pos, col, row, t->file_id, 2));
    }

    return lexer_set_curr(
        t, token_create(TOKEN_EQUAL, pos, col, row, t->file_id, 1));
  }

  if (lexer_current(t) == '!' && lexer_eat(t)) {
    if (lexer_current(t) == '=' && lexer_eat(t)) {
      return lexer_set_curr(t, token_create(TOKEN_EXCLAMATION_EQUAL, pos, col,
                                            row, t->file_id, 2));
    }
  ...
  if (lexer_can_read_word(t, "struct", &size) && lexer_eat_next_n(t, size))
    return lexer_set_curr(
        t, token_create(TOKEN_KEYWORD_STRUCT, pos, col, row, t->file_id, size));

  if (lexer_can_read_word(t, "then", &size) && lexer_eat_next_n(t, size))
    return lexer_set_curr(
        t, token_create(TOKEN_KEYWORD_THEN, pos, col, row, t->file_id, size));

  if (lexer_can_read_word(t, "resume", &size) && lexer_eat_next_n(t, size))
    return lexer_set_curr(
        t, token_create(TOKEN_KEYWORD_RESUME, pos, col, row, t->file_id, size));

  if (lexer_can_read_word(t, "i32", &size) && lexer_eat_next_n(t, size))
    return lexer_set_curr(
        t, token_create(TOKEN_I32, pos, col, row, t->file_id, size));

  if (lexer_can_read_word(t, "unit", &size) && lexer_eat_next_n(t, size))
    return lexer_set_curr(
        t, token_create(TOKEN_UNIT, pos, col, row, t->file_id, size));

  if (lexer_can_read_word(t, "type", &size) && lexer_eat_next_n(t, size))
    return lexer_set_curr(
        t, token_create(TOKEN_TYPE, pos, col, row, t->file_id, size));

  if (lexer_can_read_word(t, "continue", &size) && lexer_eat_next_n(t, size))
    return lexer_set_curr(t, token_create(TOKEN_KEYWORD_CONTINUE, pos, col, row,
                                          t->file_id, size));

  if (lexer_can_read_word(t, "with", &size) && lexer_eat_next_n(t, size))
    return lexer_set_curr(
        t, token_create(TOKEN_KEYWORD_WITH, pos, col, row, t->file_id, size));

  if (lexer_can_read_word(t, "if", &size) && lexer_eat_next_n(t, size))
    return lexer_set_curr(
        t, token_create(TOKEN_KEYWORD_IF, pos, col, row, t->file_id, size));

  if (lexer_can_read_word(t, "else", &size) && lexer_eat_next_n(t, size))
    return lexer_set_curr(
        t, token_create(TOKEN_KEYWORD_ELSE, pos, col, row, t->file_id, size));

  if (lexer_can_read_word(t, "return", &size) && lexer_eat_next_n(t, size))
    return lexer_set_curr(
        t, token_create(TOKEN_KEYWORD_RETURN, pos, col, row, t->file_id, size));

  if (lexer_can_read_word(t, "fn", &size) && lexer_eat_next_n(t, size))
    return lexer_set_curr(
        t, token_create(TOKEN_KEYWORD_FN, pos, col, row, t->file_id, size));

  if (lexer_can_read_word(t, "effect", &size) && lexer_eat_next_n(t, size))
    return lexer_set_curr(
        t, token_create(TOKEN_KEYWORD_EFFECT, pos, col, row, t->file_id, size));

  if (lexer_can_read_word(t, "handler", &size) && lexer_eat_next_n(t, size))
    return lexer_set_curr(t, token_create(TOKEN_KEYWORD_HANDLER, pos, col, row,
                                          t->file_id, size));
