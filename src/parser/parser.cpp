...
ast::Node* parse_call_args(Parser* p) {
  ...
  ast::Node* root = parser_parse_expr(p); // ast::call_arg_list(p->ast_manager, parser_curr_tok(p));
  if (root->kind != AST::DECL_ARGS_LIST) {
    root = ast::decl_args(p->ast_manager, root->tok, root, ast::node_null(p->ast_manager));
  }
...
ast::Node* parser_parse_struct(Parser* p) {
  ...
  return ast::type_struct(p->ast_manager, tok, body);
}
...
ast::Node* parser_parse_handler_literal(Parser* p) {
  ...
  ast::Node* effs = ast::program_point(p->ast_manager, parser_curr_tok(p));
  ...
  while (parser_curr_tok(p).type != TOKEN_CLOSE_CURLY_BRACE) {
    ...
    ast::Node* tail = ast::program_point(p->ast_manager, parser_curr_tok(p));
...
ast::Node* parser_parse_lit(Parser* p) {
  ...
  if (tok.type == TOKEN_I32_LIT) {
    ...
    return ast::i32_lit(p->ast_manager, tok);
  }

  if (tok.type == TOKEN_UNIT) {
    parser_read_token(p, TOKEN_UNIT);
    return ast::type_unit(p->ast_manager, tok);
  }

  if (tok.type == TOKEN_I32) {
    parser_read_token(p, TOKEN_I32);
    return ast::type_i32(p->ast_manager, tok);
  }

  if (tok.type == TOKEN_TYPE) {
    parser_read_token(p, TOKEN_TYPE);
    return ast::type_type(p->ast_manager, tok);
  }

  if (tok.type == TOKEN_CLOSE_PARENTHESIS) {
    return ast::node_null(p->ast_manager);
  }
...
ast::Node* parser_parse_primary(Parser* p) {
  if (parser_curr_tok(p).type == TOKEN_OPEN_PARENTHESIS) {
    ...
    if (root->kind == AST::DECL_ARGS_LIST || root->kind == AST::NULL_NODE || root->kind == AST::BIND_TYPE || root->kind == AST::BIND_CONSTANT
        || root->kind == AST::BIND_VARIABLE) {

      if (root->kind != AST::DECL_ARGS_LIST) {
        root = ast::decl_args(p->ast_manager, tok, root, ast::node_null(p->ast_manager));
      }

      ast::Node* type = ast::node_null(p->ast_manager);
      ...
      ast::Node* decl = ast::function_signature(p->ast_manager, tok, root, type);

      return ast::function_literal(p->ast_manager, tok, decl, body);
    }
...
ast::Node* parser_parse_call_args(Parser* p) {
  ...
  Token      tok  = parser_curr_tok(p);
  ast::Node* arg  = parse_call_args(p);
  ast::Node* tail = 0;
  ...
  if (parser_curr_tok(p).type == TOKEN_EXCLAMATION || parser_curr_tok(p).type == TOKEN_OPEN_PARENTHESIS) {
    ...
    return ast::call(p->ast_manager, tok, arg, parser_parse_call_args(p), effectfull);
  }
...
ast::Node* parser_parse_call_tail(Parser* p, ast::Node* head) {
  ...
  ast::Node* args = parse_call_args(p);

  ast::Node* call = ast::call(p->ast_manager, tok, head, args, effectfull);
...
ast::Node* parser_parse_call(Parser* p) {
  ...
  if (parser_curr_tok(p).type == TOKEN_OPEN_PARENTHESIS) {
    ...
    ast::Node* call = ast::call(p->ast_manager, tok, root, args, effectfull);
...
ast::Node* parser_parse_member_access(Parser* p) {
  ...
  if (parser_curr_tok(p).type == TOKEN_DOT) {
    ...
    return ast::member_access(p->ast_manager, parser_curr_tok(p), root, parser_parse_member_access(p));
  }
...
ast::Node* parser_parse_factor(Parser* p) {
  ...
  if (parser_curr_tok(p).type == TOKEN_SLASH || parser_curr_tok(p).type == TOKEN_ASTERISK) {
    ...
    if (tok.type == TOKEN_SLASH) {
      return ast::bin_div(p->ast_manager, tok, root, parser_parse_factor(p));
    }

    if (tok.type == TOKEN_ASTERISK) {
      return ast::bin_mul(p->ast_manager, tok, root, parser_parse_factor(p));
    }
  }
...
ast::Node* parser_parse_term(Parser* p) {
  ...
  if (parser_curr_tok(p).type == TOKEN_PLUS || parser_curr_tok(p).type == TOKEN_MINUS) {
    ...
    if (tok.type == TOKEN_PLUS) {
      return ast::bin_add(p->ast_manager, tok, root, parser_parse_term(p));
    }

    if (tok.type == TOKEN_MINUS) {
      return ast::bin_sub(p->ast_manager, tok, root, parser_parse_term(p));
    }
  }
...
ast::Node* parser_parse_comparison(Parser* p) {
  ...
  if (parser_curr_tok(p).type == TOKEN_GREATER) {
    ...
    return ast::bin_gt(p->ast_manager, curr, root, parser_parse_comparison(p));
  }
  ...
  if (parser_curr_tok(p).type == TOKEN_SMALLER) {
    ...
    return ast::bin_lt(p->ast_manager, curr, root, parser_parse_comparison(p));
  }
  ...
  if (parser_curr_tok(p).type == TOKEN_GREATER_EQUAL) {
    ...
    return ast::bin_ge(p->ast_manager, curr, root, parser_parse_comparison(p));
  }
  ...
  if (parser_curr_tok(p).type == TOKEN_SMALLER_EQUAL) {
    ...
    return ast::bin_le(p->ast_manager, curr, root, parser_parse_comparison(p));
  }
...
ast::Node* parser_parse_equality(Parser* p) {
  ...
  if (parser_curr_tok(p).type == TOKEN_EXCLAMATION_EQUAL) {
    ...
    return ast::bin_ne(p->ast_manager, curr, root, parser_parse_equality(p));
  }
  ...
  if (parser_curr_tok(p).type == TOKEN_EQUAL_EQUAL) {
    ...
    return ast::bin_eq(p->ast_manager, curr, root, parser_parse_equality(p));
  }
...
ast::Node* parser_parse_arrow(Parser* p) {
  ...
  if (/*resume ||*/ parser_curr_tok(p).type == TOKEN_ARROW) {
    ...
    tmp = ast::type_arrow(p->ast_manager, tok, tmp, tail);
...
ast::Node* parser_parse_if_statement(Parser* p) {
  ...
  ast::Node* body = ast::node_null(p->ast_manager);
  ...
  ast::Node* cont = ast::node_null(p->ast_manager);
  ...
  return ast::ctrl_flow_if(p->ast_manager, tok, cond, body, cont);
}

ast::Node* parser_parse_ret_statement(Parser* p) {
  Token tok = parser_curr_tok(p);
  parser_read_token(p, TOKEN_KEYWORD_RETURN);
  ast::Node* ret = ast::ctrl_flow_ret(p->ast_manager, tok, parser_parse_expr(p));
...
ast::Node* parser_parse_statements(Parser* p) {
  ...
  ast::Node* stmt = ast::program_point(p->ast_manager, parser_curr_tok(p));
  ...
  while (parser_curr_tok(p).type != TOKEN_CLOSE_CURLY_BRACE) {
    ...
    ast::Node* r = ast::program_point(p->ast_manager, parser_curr_tok(p));
    tmp->right   = r->id;
    tmp          = r;
  }
...
ast::Node* parser_parse_type_bind(Parser* p) {
  ast::Node* a = parser_parse_decl(p);
  ...
  if (parser_curr_tok(p).type == TOKEN_COLON) {
    ...
    if (parser_curr_tok(p).type == TOKEN_COLON || parser_curr_tok(p).type == TOKEN_EQUAL) {
      return ast::type_bind(p->ast_manager, tok, a, ast::node_null(p->ast_manager));
    }

    ast::Node* b = parser_parse_arrow(p);

    return ast::type_bind(p->ast_manager, tok, a, b);
  }
...
ast::Node* parser_parse_expr(Parser* p) {
  ast::Node* root = parser_parse_type_bind(p);
  ...
  if (parser_curr_tok(p).type == TOKEN_EQUAL) {
    ...
    if (root->kind == AST::BIND_TYPE) {
      return ast::variable_bind(p->ast_manager, parser_curr_tok(p), root, parser_parse_type_bind(p));
    }

    return ast::assignment(p->ast_manager, parser_curr_tok(p), root, parser_parse_type_bind(p));
  }
  ...
  if (parser_curr_tok(p).type == TOKEN_COLON) {
    ...
    return ast::constant_bind(p->ast_manager, tok, root, parser_parse_type_bind(p));
  }
  ...
  if (parser_curr_tok(p).type == TOKEN_COMMA) {
    ...
    if (tail->kind != AST::DECL_ARGS_LIST) {
      tail = ast::decl_args(p->ast_manager, tail->tok, tail, ast::node_null(p->ast_manager));
    }

    return ast::decl_args(p->ast_manager, tok, root, tail);
  }
...
void print_ast::rec(i8* prefix, Parser* p, ast::Node* a, b8 is_left, b8 f = 1) {
  printf("%s", prefix);
  ...
  if (a->kind == AST::NATURAL_LITERAL) {
    printf(",val: %lu", a->tok.buf);
  }

  if (a->kind == AST::SYMBOL_LITERAL) {
    i8 buff[a->tok.size + 1];

    token_get_id(p->lexer, a->tok, buff);
  ...
  strcpy(buf0, prefix);
  strcat(buf0, is_left ? "│   " : "    ");
  print_ast::rec(buf0, p, ast::manager_get_relative(p->ast_manager, a, a->left), 1);
  free(buf0);

  i8* buf1 = (i8*)malloc(strlen(prefix) + 10);
  strcpy(buf1, prefix);
  strcat(buf1, is_left ? "│   " : "    ");
  print_ast::rec(buf1, p, ast::manager_get_relative(p->ast_manager, a, a->right), 0);
  free(buf1);
}
...
b8 parser_is_same_symbol(Parser* p, ast::Node* a, ast::Node* b) {
  ...
  if (ast::is_temporary(p->ast_manager, a) && ast::is_temporary(p->ast_manager, b)) {
    return a->kind == b->kind;
  }

  assert(a->kind == AST::SYMBOL_LITERAL);
  assert(b->kind == AST::SYMBOL_LITERAL);
  ...
  for (u64 i = 0; i < t0.size; i++) {
    if (p->lexer.file_buf[t0.pos + i] != p->lexer.file_buf[t1.pos + i]) {
      return false;
    }
  }
...
void print_ast::to_program(Parser* p, ast::Node* n, u32 scope) {

  if (ast::is_temporary(p->ast_manager, n)) {
    i8* st = ast::kind_to_cstr(n->kind, __AST::KIND_END);
    printf("t%s", st);
    free(st);
    return;
  }

  if (n->kind == AST::NATURAL_LITERAL) {
    printf("%lu", n->tok.buf);
    return;
  }

  if (n->kind == AST::SYMBOL_LITERAL) {
    i8 buff[n->tok.size + 1];

    token_get_id(p->lexer, n->tok, buff);

    printf("%s", buff);
    return;
  }

  if (n->kind == AST::DECL_ARGS_LIST) {
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->left), scope);
    if (n->right)
      printf(", ");
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->right), scope);
    return;
  }
  if (n->kind == AST::TYPE_UNIT) {
    printf("unit");
    return;
  }

  if (n->kind == AST::TYPE_I32) {
    printf("i32");
    return;
  }
  if (n->kind == AST::TYPE_ARROW) {
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->left), scope);
    printf(" -> ");
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->right), scope);
    return;
  ...
  if (n->kind == AST::TYPE_ANY) {
    printf("any");
    return;
  }

  if (n->kind == AST::FUN_SIGNATURE) {
    printf("(");
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->left), scope);
    printf(")");
    if (n->right)
  ...
  if (n->kind == AST::FUNCTION_LITERAL) {
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->left), scope);
    printf(" {\n");
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->right), scope + 2);
  ...
  if (n->kind == AST::CTRL_FLOW_RETURN) {
    printf("return ");
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->left), scope);
    printf("");
    return;
  ...
  if (n->kind == AST::CTRL_FLOW_IF_ELSE) {
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->left), scope);
    for (i32 i = 0; i < scope; i++)
      printf(" ");
  ...
  if (n->kind == AST::UNDEFINED_NODE) {
    printf("undefined");
    return;
  }
  if (n->kind == AST::UNITIALIZED_NODE) {
    printf(".{}");
    return;
  }

  if (n->kind == AST::TYPE_POINTER) {
    printf("ptr");
    printf("[");
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->left), scope);
    printf("]");
  ...
  if (n->kind == AST::OP_MEMBER_ACCESS) {
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->left), scope);
    printf(".");
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->right), scope);
    return;
  ...
  if (n->kind == AST::TYPE_STRUCT) {
    printf("struct {\n");
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->left), scope + 2);
    for (i32 i = 0; i < scope; i++)
      printf(" ");
  ...
  if (n->kind == AST::OP_POINTER_LOAD) {
    printf("ld[");
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->left), scope);
    printf("]");
    return;
  }
  if (n->kind == AST::OP_ADDRESS_OF) {
    printf("$");
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->left), scope);
    printf("");
    return;
  }
  // if (n->kind == _AST::BUILD_STACK_CLOSURE_OBJECT) {
  ...
  // if (n->kind == _AST::BUILD_HEAP_CLOSURE_OBJECT) {
  ...
  if (n->kind == _AST::GET_CLOSURE_HANDLER) {
  ...
  if (n->kind == _AST::CAPTURE_VARIABLE_INTO_ENVIRONMENT) {
  ...
  if (n->kind == _AST::BORROW_VARIABLE_INTO_ENVIRONMENT) {
  ...
  if (n->kind == _AST::GET_CLOSURE_ENVIRONMENT_BUFFER) {
  ...
  if (n->kind == _AST::GET_CLOSURE_ENVIRONMENT_BUFFER_SIZE) {
  ...
  if (n->kind == _AST::UPDATE_CLOSURE_ENVIRONMENT_BUFFER_HEADER) {
  ...
  if (n->kind == _AST::SETUP_CLOSURE_ENVIRONMENT_BUFFER_HEADER) {
  ...
  if (n->kind == _AST::BITSET) {
    printf("_bitset(%lu)", n->tok.buf);
    return;
  }

  if (n->kind == _AST::BITSET_SET_BIT_ON) {
    printf("_bitset_set_bit_on(");
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->left));
    printf(", %lu)", n->tok.buf);
    return;
  }

  if (n->kind == _AST::BITSET_SET_BIT_OFF) {
    printf("_bitset_set_bit_off(");
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->left));
    printf(", %lu)", n->tok.buf);
    return;
  }
  if (n->kind == _AST::BITSET_IS_BIT_UP) {
  ...
  if (n->kind == _AST::BUILD_CLOSURE_OBJECT) {
  ...
  if (n->kind == _AST::BITSET_BINARY_UNION) {
    printf("_bitset_binary_union(");
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->left));
    printf(", ");
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->right));
    printf(")");
    return;
  }

  if (n->kind == _AST::BITSET_BINARY_INTERSECTION) {
  ...
  if (n->kind == _AST::ALLOCATE_HEAP_BUFFER) {
    printf("_malloc(");
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->left));
    printf(")");
    return;
  ...
  if (n->kind == _AST::IS_CLOSURE_OBJECT_LOCAL) {
  ...
  if (n->kind == _AST::GET_CLOSURE_OBJECT_BITSET) {
  ...
  if (n->kind == _AST::SET_CLOSURE_OBJECT_EXTERN) {
  ...
  if (n->kind == _AST::REALLOCATE_HEAP_BUFFER) {
    printf("_realloc(");
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->left));
    printf(")");
    return;
  ...
  if (n->kind == AST::OP_BIN_NE) {
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->left), scope);
    printf(" != ");
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->right), scope);
    return;
  }
  if (n->kind == AST::OP_BIN_GT) {
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->left), scope);
    printf(" > ");
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->right), scope);
    return;
  }
  if (n->kind == AST::OP_BIN_GE) {
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->left), scope);
    printf(" >= ");
    print_ast::to_program(p, ast::manager_get_relative(p->ast_manager, n, n->right), scope);
    return;
